...
SONAR_URL=http://dist.sonar.codehaus.org/sonar-${APP_VERSION}.zip
...
if [ ! -f SOURCES/downloaded/$APP_NAME-${APP_VERSION}.zip ]; then
  echo "downloading sonar-${APP_VERSION}.zip from $SONAR_URL"
  curl -s -L $SONAR_URL -o SOURCES/downloaded/$APP_NAME-${APP_VERSION}.zip
fi
