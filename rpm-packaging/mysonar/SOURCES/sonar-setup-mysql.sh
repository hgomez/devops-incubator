...
GRANT ALL ON $SONAR_USER.* TO '$SONAR_DATABASE'@'%' IDENTIFIED BY '$SONAR_PASSWORD';
GRANT ALL ON $SONAR_USER.* TO '$SONAR_DATABASE'@'localhost' IDENTIFIED BY '$SONAR_PASSWORD';
FLUSH PRIVILEGES;
EOF1
