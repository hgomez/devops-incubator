...
if [ ! -f SOURCES/artifactory-${ARTIFACTORY_VERSION}.zip ]; then
  echo "downloading artifactory-${ARTIFACTORY_VERSION}.zip from $ARTIFACTORY_URL"
  curl -s -L $ARTIFACTORY_URL -o SOURCES/artifactory-${ARTIFACTORY_VERSION}.zip
fi
